...
for Nhidx = 1:length(NhSeq)
    ...
    for tidx = 1:length(timeSeq)
        ...
        teacherPrediction = zeros(numSubject,trialLength*freqLength,freqLength);
        parfor targetSubject = 1:numSubject
            ssvep = allData{targetSubject};
            rec_subjectSpecific = zeros(freqLength);
            scoreReg = zeros(trialLength,freqLength,freqLength);
            for trial = 1:trialLength
                XTemplate = squeeze(mean(ssvep(trialSeq ~= trial,:,startIdx+1:startIdx+floor(time*fsample),:)));
                for freq = 1:freqLength
                    ...
                    scoreReg(trial,freq,:) = score;
                end
            end
            teacherPrediction(targetSubject,:,:) = reshape(scoreReg,trialLength*freqLength,freqLength);
        ...
        str = sprintf('%s_%s_Nh%d_time%d_filter%d_TeacherLabel',method,nameDataset,Nh,time*100,1);
        save(['Feature\' str '.mat'],'teacherPrediction');
        toc
    end
    %%  Save result to xls
